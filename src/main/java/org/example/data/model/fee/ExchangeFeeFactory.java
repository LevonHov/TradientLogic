...
public class ExchangeFeeFactory {
    ...
    private void initializeDefaultFees() {
        // Binance fees (0.10% maker, 0.10% taker)
     ...
     * @param bnbPayment Whether BNB payment discount is applied
     * @return A Fee object with the appropriate structure
     */
    public Fee createBinanceFee(double thirtyDayVolume, boolean isMaker, boolean bnbPayment) {
        // Define VIP tier thresholds and rates
        ...
        // If BNB payment is enabled, apply the 25% discount
        if (bnbPayment) {
            // This will be a wrapper that applies the discount
            final double discountRate = 0.25; // 25% discount
            final TieredFee finalTieredFee = tieredFee;
            
            // Return a fee implementation that applies the discount
            return new Fee() {
                @Override
                public double calculateFee(double amount) {
                    return finalTieredFee.calculateFee(amount) * (1 - discountRate);
                }
                
                @Override
                public String getDescription() {
                    return finalTieredFee.getDescription() + " with 25% BNB discount";
                }
    ...
    public Fee createFee(String exchangeName, double thirtyDayVolume, boolean isMaker, boolean bnbPayment) {
        switch (exchangeName) {
            case "Binance":
                return createBinanceFee(thirtyDayVolume, isMaker, bnbPayment);
            case "Coinbase":
    ...
    public Fee createFee(String exchangeName, double thirtyDayVolume, boolean isMaker) {
        return createFee(exchangeName, thirtyDayVolume, isMaker, false);
    }
    ...
    public Fee createFee(String exchangeName, boolean isMaker) {
        return createFee(exchangeName, 0.0, isMaker, false);
    }
} 